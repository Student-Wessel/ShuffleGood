...
// Class is a copy of UnityEngine.InputSystem.OnScreen but with some adjustments
public class UIJoystick : OnScreenControl, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    [SerializeField]
    public UnityEvent<Vector2> JoystickAimChangeEvent;

    [SerializeField]
    public UnityEvent<Vector2> JoystickReleasedEvent;
    ...
    private void MoveStick(Vector2 pointerPosition, Camera uiCamera)
    {
        ...
        Vector2 deltaScaled = delta.normalized * (delta.magnitude/m_MovementRange);
        JoystickAimChangeEvent?.Invoke(deltaScaled);
        SendValueToControl(newPos);
    }

    private void EndInteraction()
    {
        StartCoroutine(FadeToStartPos());
        Vector2 delta = ((RectTransform)transform).anchoredPosition - (Vector2)m_StartPos;
        Vector2 deltaScaled = delta.normalized * (delta.magnitude/m_MovementRange);
        JoystickReleasedEvent?.Invoke(deltaScaled);
        SendValueToControl(Vector2.zero);
    }

    private IEnumerator FadeToStartPos()
    {
        float timePast = 0;
        fading = true;
        float distance = Vector2.Distance(((RectTransform)transform).anchoredPosition, m_StartPos);
        float speed = distance / m_animationTime; // calculate the speed based on the distance and animation time
        while (true)
        {
            yield return new WaitForSeconds(Time.deltaTime);
            timePast += Time.deltaTime;
            float t = timePast / m_animationTime; // calculate the interpolation factor based on time elapsed and animation time
            ((RectTransform)transform).anchoredPosition = Vector2.Lerp(((RectTransform)transform).anchoredPosition, m_StartPos, t * speed);
            if (Vector2.Distance(((RectTransform)transform).anchoredPosition, m_StartPos) < 0.01f)
            {
    ...
    internal class OnScreenStickEditor : UnityEditor.Editor
    {
        ...
        public void OnEnable()
        {
            ...
            m_UseIsolatedInputActions = serializedObject.FindProperty(nameof(UIJoystick.m_UseIsolatedInputActions));

            m_Behaviour = serializedObject.FindProperty(nameof(UIJoystick.m_Behaviour));
            m_ControlPathInternal = serializedObject.FindProperty(nameof(UIJoystick.m_ControlPath));
            m_MovementRange = serializedObject.FindProperty(nameof(UIJoystick.m_MovementRange));
            m_DynamicOriginRange = serializedObject.FindProperty(nameof(UIJoystick.m_DynamicOriginRange));
            m_PointerDownAction = serializedObject.FindProperty(nameof(UIJoystick.m_PointerDownAction));
            m_PointerMoveAction = serializedObject.FindProperty(nameof(UIJoystick.m_PointerMoveAction));
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            m_ShowDynamicOriginOptions.target = ((UIJoystick)target).behaviour ==
            ...
            if (EditorGUILayout.BeginFadeGroup(m_ShowDynamicOriginOptions.faded))
            {
                ...
                {
                    ((UIJoystick)target).UpdateDynamicOriginClickableArea();
                }
